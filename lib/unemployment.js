...
function fetch (opt, next) {
  ...
  request({
    ...
    if (body.message && body.message.length > 0) {
      // If there's no data for the start or year yet, this will bomb. Causes problems in January.
      ...
      } else if (body.message[0] == 'Start year is invalid.') {
        return fetch({
...
function previousYear (year) {
  return moment(year, 'YYYY').subtract(1, 'years').format('YYYY')
}
